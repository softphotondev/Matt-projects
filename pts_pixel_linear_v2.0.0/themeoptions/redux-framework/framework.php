        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'azkaban_options' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'azkaban_options' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'azkaban_options' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'azkaban_options' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    if ( $this->args['dev_mode'] == true || Redux_Helpers::isLocalHost() == true ) {
                        get_template_part('themeoptions/redux-framework/core/dashboard');
                        new reduxDashboardWidget( $this );

                        if ( ! isset ( $GLOBALS['redux_notice_check'] ) || $GLOBALS['redux_notice_check'] == 0 ) {
                            get_template_part('themeoptions/redux-framework/core/newsflash');
            ...
            private function _internationalization() {
                 ...
                 * @param string     'azkaban_options'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'azkaban_options' );
                //
                ...
                $loaded = load_plugin_textdomain( 'azkaban_options', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'azkaban_options', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'azkaban_options', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'azkaban_options' );
                    $mofile = dirname( __FILE__ ) . '/languages/redux-framework-' . $locale . '.mo';
                    load_textdomain( 'azkaban_options', $mofile );
                }
            }
...
//                    }
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
                        ...
                        $this->page = add_theme_page( $page_title, $menu_title, $page_permissions, $page_slug );

						/* call_user_func( 'add_submenu_page', $page_parent, $page_title, $menu_title, $page_permissions, $page_slug, array(
                            &$this,
                            'generate_panel'
                        ) ); */
                    }
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        foreach ( $this->sections as $k => $section ) {
                            ...
                            // ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
							add_theme_page( $section['title'], $section['title'], $this->args['page_permissions'], $this->args['page_slug'] . '&tab=' . $k, '__return_null' );
            ...
            public function _enqueue() {
                get_template_part('themeoptions/redux-framework/core/enqueue');
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'azkaban_options' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'azkaban_options' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'azkaban_options' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'azkaban_options' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'azkaban_options' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'azkaban_options' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'azkaban_options' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'azkaban_options' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'azkaban_options' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'azkaban_options' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        get_template_part('themeoptions/redux-framework/core/panel');
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'azkaban_options' );
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if (!$this->omit_share_icons) {
                                        if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                            $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'azkaban_options' );
