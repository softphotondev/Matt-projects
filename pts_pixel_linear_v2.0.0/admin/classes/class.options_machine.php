...
class Options_Machine {
				...
				case 'slider':
					...
					if ($count < 2) {
						...
						foreach ($slides as $slide) {
							$oldorder = isset($slide['order'])?$slide['order']:1;
							$i++;
							$order = $i;
							$output .= Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order);
						}
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		$uploader .= '<input class="'.$hide.' upload of-input" name="'. $id .'" id="'. $id .'_upload" value="'. $val['url'] .'" />';
		...
		if(!empty($upload)){	
	    	$uploader .= '<a class="of-uploaded-image" href="'. $upload['url'] . '">';
	    	$uploader .= '<img class="of-option-image" id="image_'.$id.'" src="'.$upload['url'].'" alt="" />';
	    	$uploader .= '</a>';			
			}
		$uploader .= '</div>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if( isset($val['title']) && $val['title'] != '' ){
			$dd_slider_title = $val['title'];
		}
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes($dd_slider_title) .'" />';
		
		$slider .= '<label>Image URL</label>';
		$dd_slider_url = '';
		if( isset($val['url']) && $val['url'] != '' ){
			$dd_slider_url = $val['url'];
		}
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $dd_slider_url .'" />';
		...
		if( isset($val['link']) && $val['link'] != '' ){
			$dd_slider_link = $val['link'];
		}
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $dd_slider_link .'" />';
		
		$slider .= '<label>Description (optional)</label>';
		$dd_slider_description = '';
		if( isset($val['description']) && $val['description'] != '' ){
			$dd_slider_description = $val['description'];
		}
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($dd_slider_description).'</textarea>';
